...
export default function CoruselBanner() {
                        ...
                        return (
                            <CarouselItem key={idx}>
                                <Link href={`/articles/${post.slug}`} className="block group h-90 md:h-110 p-3 md:p-5 relative w-full bg-card rounded-lg overflow-hidden">
                                    {
                                        post.image ? <Image loading='lazy' src={urlFor(post.image).url()} alt={post.title || categoryTitle} fill className="object-cover group-hover:scale-115 duration-500" />
                                        : <div className='w-full h-full bg-foreground/5' />
                                    }
                                    <div className='absolute z-10 px-3 py-1.5 bg-white rounded-sm'>
                                        <p className='text-xs uppercase font-medium tracking-wide'>{categoryTitle}</p>
                                    </div>
